use crate::{BoofSocket, serialization::{serialize, deserialize, serialized_size}};
...
impl Connection {
    ...
    pub fn send_bytes(&mut self, bytes: &[u8]) -> Result<(), std::io::Error> {
        ...
        let header = Header { seq: self.sequence, ack: self.ack, acks: self.acks };
    ...
    pub fn receive_bytes(&mut self) -> Result<Option<Vec<u8>>, std::io::Error> {
        ...
        for b in (0u32..32u32).rev() {
            let s = header.ack as i32 - b as i32;
