struct VertexInput {
    @location(0)  position: vec2<f32>,
}

struct VertexOutput {
    @builtin(position) position: vec4<f32>,
}

//@group(2) @binding(0) var<uniform> resolution: vec2<f32>;

@vertex
fn vs_main(
    in: VertexInput
) -> VertexOutput {
    var out: VertexOutput;

    out.position = vec4(in.position, 0.0, 1.0);

    return out;
}

// Fragment shader
@group(0) @binding(0) var<uniform> viewproj: mat4x4<f32>;
@group(0) @binding(1) var<uniform> near: f32;
@group(0) @binding(2) var<uniform> far: f32;
@group(1) @binding(0) var voxel_data: texture_3d<u32>;
@group(2) @binding(0) var palette: texture_1d<f32>;

fn voxel_from_pos(position: vec3<i32>) -> vec4<f32> {
    if(
        0 <= position.x && position.x < 32 &&
        0 <= position.y && position.y < 32 &&
        0 <= position.z && position.z < 32
    ){
        //calculate our tex coord from our y, and z, x coords are contained in one pixel that is 32 bit
        let offset = vec3<u32>(vec2<u32>(position.xy) % vec2<u32>(u32(2)), u32(0));
        let actual_position = position / vec3<i32>(2, 2, 1);
        let pixel = textureLoad(voxel_data, actual_position.xyz, 0).r;

        let sub_pixel_offset = offset.x | (offset.y << u32(1));

        let color_index = i32((pixel & (u32(0xFF) << (sub_pixel_offset * u32(8)))) >> (sub_pixel_offset * u32(8)));

        if color_index > 0 {
             return textureLoad(palette, color_index, 0);
        }

    }

    return vec4<f32>(-1.0);
}

@fragment
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    //should vectorize all these operations, no reason to be this verbose
    let step = vec3<i32>(sign(in.ray));
    let t_delta = 1.0 / in.ray;

    let world_fract = abs(fract(in.origin));

    var t_max_x = 0.0;
    if(in.ray.x > 0.0){
        t_max_x = 1.0 - world_fract.x;
    } else {
        t_max_x = world_fract.x;
    };

    var t_max_y = 0.0;
    if(in.ray.y > 0.0){
        t_max_y = 1.0 - world_fract.y;
    } else {
        t_max_y = world_fract.y;
    };

    var t_max_z = 0.0;
    if(in.ray.z > 0.0){
        t_max_z = 1.0 - world_fract.z;
    } else {
        t_max_z = world_fract.z;
    };

    var t_max = t_delta * vec3<f32>(t_max_x, t_max_y, t_max_z);

    var voxel = vec3<i32>(floor(in.origin));

    //we now have a starting voxel we should check for collision???

    //cap the number of iterations at zero
    var iterations = 0.0;
    for(var i: i32; i < 50; i++) {
        iterations += 0.02;
        if abs(t_max.x) < abs(t_max.y) && abs(t_max.x) < abs(t_max.z) {
            t_max.x = t_max.x + t_delta.x;
            voxel.x += step.x;
        } else if abs(t_max.y) < abs(t_max.z) {
            t_max.y= t_max.y + t_delta.y;
            voxel.y += step.y;
        } else {
            t_max.z = t_max.z + t_delta.z;
            voxel.z += step.z;
        }

        let color = voxel_from_pos(voxel);
        if color.x > -0.1 {
            return color;
        }
    }

    return vec4<f32>(0.0);
}
